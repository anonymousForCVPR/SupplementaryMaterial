...
class SphereCrop(object):
    def __init__(self, point_max=80000, sample_rate=None, mode="random", as_cube=False):
        self.point_max = point_max
        self.sample_rate = sample_rate
        assert mode in ["random", "center", "all"]
        self.mode = mode
        self.as_cube = as_cube

    def __call__(self, data_dict):
        if isinstance(self.point_max, (list, tuple)):
            point_max = np.random.randint(self.point_max[0], self.point_max[1])
        else:
            point_max = (
        ...
        # mode is "random" or "center"
            ...
            if self.mode == "random":
                ...
                raise NotImplementedError
            dist_fn = np.abs if self.as_cube else np.square
            idx_crop = np.argsort(np.sum(dist_fn(data_dict["coord"] - center), 1))[
                :point_max
            ]
            if "coord" in data_dict.keys():
