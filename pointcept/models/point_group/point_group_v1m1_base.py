...
class PointGroup(nn.Module):
    def __init__(
        ...
        voxel_size=0.02,
        use_abs_similarity=False,
    ):
        self.use_abs_similarity = use_abs_similarity
        super().__init__()
    ...
    def forward(self, data_dict):
        ...
        bias_pred = self.bias_head(point.feat)
        logit_pred = self.seg_head(point.feat)
        ...
        if self.use_abs_similarity:
            cosine_similarity = (bias_pred_norm * bias_gt_norm).sum(-1).abs()
        else:
        ...
        if self.use_abs_similarity:
            return_dict['bias_abscos_loss'] = bias_cosine_loss
        else:
