    ...
    def eval(self):
        ...
        for i, input_dict in enumerate(self.trainer.val_loader):
            ...
            if size <= THRES:
                ...
                device = input_dict["feat"].device
                # # random split points to inference
                # num_splits = math.ceil(size / THRES)
                # perm_indices = torch.randperm(size, device=device)
                # split_size = math.ceil(size / num_splits)
                # split_indices = []
                # for i in range(num_splits):
                #     start = i * split_size
                #     end = min((i + 1) * split_size, size)
                #     split_indices.append(perm_indices[start:end])
                # sobel sample and cube gather
                valid = False
                mult = 2
                while not valid:
                    num_samples = math.ceil(mult * size / THRES)
                    sobol_sampler = qmc.Sobol(3, scramble=True)
                    samples = sobol_sampler.random(num_samples)
                    samples = torch.from_numpy(samples).to(device)
                    pc_start = input_dict["coord"].min(0).values
                    pc_range = input_dict["coord"].max(0).values - pc_start
                    sample_center = pc_start + samples * pc_range
                    distance = input_dict["coord"].unsqueeze(1) - sample_center.unsqueeze(0)
                    inds = distance.abs().sum(dim=-1).topk(THRES, dim=0, largest=False).indices
                    split_indices = [inds[:, i] for i in range(num_samples)]
                    sampled_cnt = torch.zeros(size, device=device)
                    for i in range(num_samples):
                        sampled_cnt[split_indices[i]] += 1
                    valid = (sampled_cnt > 0).int().sum() == size
                    if not valid:
                        mult += 1
                # compute net
                loss = 0
                ...
                for indices in split_indices:
                    torch.cuda.empty_cache()
                    sub_input_dict = dict(
                        feat=input_dict["feat"][indices],
                        segment=input_dict["segment"][indices],
                        coord=input_dict["coord"][indices],
                        grid_coord=input_dict["grid_coord"][indices],
                        offset=input_dict["offset"].new([indices.shape[0]]),
                    )
                    with torch.no_grad():
                        output_dict = self.trainer.model(sub_input_dict)
                    output = output_dict["seg_logits"]
                    if outputs is None:
                        outputs = torch.zeros((size, output.shape[1]), device=device)
                    outputs[indices] += output
                    loss += output_dict["loss"]
                loss /= len(split_indices)
