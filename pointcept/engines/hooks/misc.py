...
@HOOKS.register_module()
class DatasetInfoLogger(HookBase):
    def __init__(self) -> None:
        super().__init__()

    def before_train(self):
        assert isinstance(
            self.trainer.train_loader.dataset, 
            (S3DISDataset, ScanNetDataset)
        )
        ds = self.trainer.train_loader.dataset
        ds.log_info(self.trainer.logger)


@HOOKS.register_module()
class ModelWeightEMA(HookBase):
    def __init__(self, decay=0.999) -> None:
        super().__init__()
        self.decay = decay
        self.ema_weights = {}
        self.backup = {}
    ...
    def before_epoch(self):
        # restore weight to original weight
